...
//    OLED_DisplayTurn(0);//0姝ｅ父鏄剧ず 1 灞忓箷缈昏浆鏄剧ず
    ...
    HAL_Delay(500);
    OLED_Clear();
    OLED_ShowChinese(0,0,0,16);  //16*16 涓?
    OLED_ShowChinese(16,0,0,24); //24*24 涓?
    OLED_ShowChinese(24,20,0,32);//32*32 涓?
    OLED_ShowChinese(64,0,0,64); //64*64 涓?
    OLED_Refresh();
    HAL_Delay(500);
    OLED_Clear();
    OLED_ShowString(0,0,"ABC",12);//6*12 鈥淎BC鈥?
    OLED_ShowString(0,12,"ABC",16);//8*16 鈥淎BC鈥?
    OLED_ShowString(0,28,"ABC",24);//12*24 鈥淎BC鈥?
    OLED_Refresh();
    HAL_Delay(500);
